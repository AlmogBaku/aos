# AGENTS — example-base root contract
...
|---|---|---|---|---|---|---|
| user | `**` | read write grant | — | 2026-07-27 | — | root of authority |
| agent:archiver | `_raw/**` | write route-into | user | 2026-07-27 | kb@0.7.0 | immutable once ingested; sha256 dedup |
| agent:archiver | `entities/** concepts/** projects/**` | write | user | 2026-07-27 | kb@0.7.0 | wiki synthesis — default-empty promotion |
| agent:archiver | `.kb/pending/** index.md` | write | user | 2026-07-27 | kb@0.7.0 | the pending queue is one file per entry |
| agent:main | `_raw/** .kb/pending/**` | write route-into | user | 2026-07-27 | kb@0.7.0 | the live capture path (`kb capture`) |
| agent:main | `.kb/state/**` | write | user | 2026-07-27 | kb@0.7.0 | THE single state writer (`kb state`); one shard per principal |
| agent:main | `profile/**` | write | user | 2026-07-27 | kb@0.7.0 | high-stakes; surface changes to the user |
| `*` | `**` | read | user | 2026-07-27 | kb@0.7.0 | registered agents read everything |
...
manual — `rebase-5min`: `kb sync` runs from the harness cron, no LLM in the

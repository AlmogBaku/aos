...
Scaled-down version of the `capability-build` flow — skip Intake (the capability and the
...
- **Apply** — same materialization rules as `capability-build`'s Build stage: names per
  ...
  per the contribution mechanics in `reference/contribute.md` (the skill links it). Bump

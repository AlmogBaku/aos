...
- "The drainer's cron should run at 22:30, not 23:00" — a personalization answer
  (`drain_hour`): update it through the `capability-onboard` skill, then sync the live cron to
...
- "Capture entries should keep the raw timestamp" — a wording/format tweak inside an
  ...
  branch and offer the contribution (reference/contribute.md; local divergence only
  if the user knowingly accepts it). For a capability the user authored, the same
...
- **A knob promotion** — the evolver hands over the MOD statement "captures after 22:00
  ...
  names. Small: nothing new is owned; the ledger search (contribute.md) decides
  issue vs PR.
...
- "The drainer should also post a weekly summary to Slack" — a new schedule, a new
  ...
  is drafted, and the maintenance-willingness question applies (overlay.md's

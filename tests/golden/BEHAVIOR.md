...
| 7 | Sync conflict (tool, scratch clone of personal-kb with a bare remote) | Exit 3, rebase aborted clean, `sync-conflict` log line, `_ops/needs-review/` block; `git status` shows no rebase in progress; **no Hermes agent invocation appears in any profile log during the sync** |
...
| 10 | Drain (gtd-capture): with pending captures seeded (step 1), drive the drainer's nightly-drain (`hermes -p aos-drainer -z` or fallback) BEFORE kb's promote | An actionable capture produces a next-action (`_ops/next-actions.md` or a project's `next_action` field); `meta.gtd_triaged: true` set with the capture's `triage` still `pending` and all other frontmatter byte-identical; failed count surfaced; **`base lint` grants audit stays clean afterwards** (the drainer's three grant rows cover every path it touched) |

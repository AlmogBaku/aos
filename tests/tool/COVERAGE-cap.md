...
- [ ] test_a_dot_aos_without_capability_roots_is_not_a_household — PIN: `find_home_soft` accepts any ancestor with a `.aos/`, and `~/aos/.aos/` exists on every machine that has installed aos, so linting a bare kit clone claimed it checked a household holding zero capabilities — **PIN**; 0; a skipped source must never be indistinguishable from an empty one, so the report says `NO HOUSEHOLD RESOLVED` / `NO --harness-skills GIVEN` in as many words

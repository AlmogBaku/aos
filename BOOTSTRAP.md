...
1. [D] Read its `CAPABILITY.md`; host check (`cron: preferred` — if absent, note the
   degraded mode for each schedule, per cheat-sheet). Install the tool first:
...
3. [A] Materialize: `main`-scoped skills to the front agent; the `kb` entry skill also goes
   ...
   (the map + reference docs; the tool itself is the installed `base` command — lint is a tool verb now, not its own skill)
   into the archiver's workspace too.

...
- **No program anywhere** — `aos import` / `install` / `update` are conversational actions the harness agent performs, never a CLI. Per-harness support is a cheat-sheet (`harnesses/<harness-runtime>.md` — knowledge), never adapter code. Capability-shipped software is standalone behind a process boundary, reached by a thin per-harness shim.

...
estimate: 45m
block: 2026-08-01T10:00
slipped: 0              # reschedules, for the escalation threshold
since: 2026-07-27       # last movement, for stall detection
waiting_on: Robin       # only when status: waiting
expires: 2026-10-15     # set on completion. The ONLY field kb acts on
```
...
The rule has a cost, and paying it is {{skill: update}}'s third step: when a commitment
...
An action page is mutable by design — {{skill: update}} flips `status`, the steward increments

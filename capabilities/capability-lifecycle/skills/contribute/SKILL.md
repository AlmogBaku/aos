...
behaves *for this user* is {{skill: evolve}} — the overlay,
not the source.
...
- **A promotion** (routed from {{skill: evolve}} with a MOD statement, or "promote
...
- **Small**: apply where the feedback actually lives, and make it take effect now —
  a personalization answer changes through the {{skill: onboard}} skill (the only MOD.md
...
- **Major**: interrupt like {{skill: build}}'s detector does, then run the
